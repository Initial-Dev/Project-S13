...
async function uploadDirectoryToS3(directoryPath: string, s3BucketName: string, s3FolderName: string) {
  ...
  const uploadPromises = filesList.map(async (file: any) => {
    ...
    if (file.isDirectory()) {
      await uploadDirectoryToS3(filePath, s3BucketName, `${s3FolderName}/${file.name}`);
    } else {
